...
RUN cd /workspace/StreamVision-Client && \
	mkdir build && cd build && \
	cmake .. -DBUILD_DOCS=ON && \
	make -j$(nproc)

CMD ["bash", "-c", "python3 -m http.server 8001 --directory /workspace/StreamVision-Client/docs/html >/dev/null 2>&1 & exec /bin/bash"]

WORKDIR /workspace/StreamVision-Client/build
